...
const IssueCard = ({
  ...
  return (
    ...
    >
      <p className="text-xs font-medium text-slate-500 mb-2">{projectName}</p>
        ...
        <div className="space-y-4">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            setSelectedSprints(
                              selectedSprints.filter((id) => id !== sprint._id)
                            );
                          }
                        }}
                        className="rounded flex-shrink-0"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              ...
              <div className="space-y-3">
                {filteredIssues["To Do"].map((issue, idx) => {
                  const { key, project, ...issueProps } = issue;
                  return (
                    <IssueCard
                      key={idx}
                      projectName={project?.name || "Unknown Project"}
                      {...issueProps}
                      onClick={() => handleShowIssueOverview(issue)}
                      onDragStart={(e) => handleDragStart(e, issue, "To Do")}
            ...
            >
              ...
              <div className="space-y-3">
                {filteredIssues["In Progress"].map((issue, idx) => {
                  const { key, project, ...issueProps } = issue;
                  return (
                    <IssueCard
                      key={idx}
                      ...
                      onClick={() => handleShowIssueOverview(issue)}
                      onDragStart={(e) =>
                        handleDragStart(e, issue, "In Progress")
                      }
                      isDragging={draggedIssue?.key === issue.key}
                    />
                  );
                ...
                {filteredIssues["Review"].map((issue, idx) => {
                  ...
                  return (
                    <IssueCard
                      ...
                      {...issueProps}
                      onClick={() => handleShowIssueOverview(issue)}
                      onDragStart={(e) => handleDragStart(e, issue, "Review")}
                      isDragging={draggedIssue?.key === issue.key}
                    />
                  );
                })}
                ...
                {filteredIssues["Done"].map((issue, idx) => {
                  ...
                  return (
                    <IssueCard
                      ...
                      {...issueProps}
                      onClick={() => handleShowIssueOverview(issue)}
                      onDragStart={(e) => handleDragStart(e, issue, "Done")}
                      isDragging={draggedIssue?.key === issue.key}
                    />
                  );
                })}
